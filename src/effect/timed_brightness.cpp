...
namespace kivsee_render
{
    namespace effect
    {
        void TimedBrightness::Render(float rel_time, int cycle_index)
        {
        ...
        bool TimedBrightness::InitFromPb(pb_istream_t *stream, const pb_field_t *field, void **arg)
        {
            kivsee_proto_render_v1_TimedBrightnessEffectConfig config = kivsee_proto_render_v1_TimedBrightnessEffectConfig_init_zero;
            ...
            if (!pb_decode(stream, kivsee_proto_render_v1_TimedBrightnessEffectConfig_fields, &config))
            {
                return false;
            }
            ...
            *((Effect **)*arg) = new TimedBrightness(mult_factor_increase, mult_factor_decrease);
            return true;
        }
