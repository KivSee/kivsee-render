...
namespace kivsee_render
{
    namespace effect
    {
        void TimedHue::Render(float rel_time, int cycle_index)
        {
        ...
        bool TimedHue::InitFromPb(pb_istream_t *stream, const pb_field_t *field, void **arg)
        {
            kivsee_proto_render_v1_TimedHueEffectConfig config = kivsee_proto_render_v1_TimedHueEffectConfig_init_zero;
            ...
            if (!pb_decode(stream, kivsee_proto_render_v1_TimedHueEffectConfig_fields, &config))
            {
                return false;
            }
            *((Effect **)*arg) = new TimedHue(offset_factor);
            return true;
        }
