...
// static std::vector<kivsee_render::HSV> leds_buffer;
...
//         int valuesToCopy = std::min(arraySize / 3, static_cast<int>(leds_buffer.size()));
        
//         // Copy HSV values to the provided array (hue, sat, val, hue, sat, val, ...)
//         for (int i = 0; i < valuesToCopy; i++) {
//             hsvArray[i * 3] = leds_buffer[i].hue;
//             hsvArray[i * 3 + 1] = leds_buffer[i].sat;
//             hsvArray[i * 3 + 2] = leds_buffer[i].val;
//         }
